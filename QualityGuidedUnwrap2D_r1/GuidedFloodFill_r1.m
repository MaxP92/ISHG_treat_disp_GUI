function IM_unwrapped = GuidedFloodFill_r1(IM_phase, IM_mag, IM_unwrapped, unwrapped_binary, derivative_variance, adjoin, IM_mask)
...
while sum(sum(adjoin(:))) ~= 0  %Loop until there are no more adjoining pixels
    if ~mod(ct,1000)
        fprintf('Ct = %d/%d\n',ct, size(IM_phase,1)*size(IM_phase,2));
    end
